setwd("D:\\as-code\\1D\\specific\\hete")
...
noise=rep(n.level*(max(fundet(t))-min(fundet(t))),length(t))
set.seed(n.level*115)#7
noise.sd=rnorm(length(t),mean=noise,sd=0.07)
...
#################define data frame
data_MSE=matrix(,1,7)
colnames(data_MSE)=c("0","0.01","0.02","0.1","0.15","0.2","0.25")#######preset noise level prefactor phi
...
data_nosie_level=as.data.frame(c(0,0.01,0.02,0.1,0.15,0.2,0.25))#######preset noise level prefactor phi
...
write.csv(data.MSE,"data_MSE_hete.csv")