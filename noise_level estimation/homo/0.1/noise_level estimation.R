setwd("D:\\as-code\\1D\\noise_level estimation\\homo\\0.1")
...
colnames(y_all)="y"
#plot(unlist(x_all),unlist(y_all),type="l)
############set homogeneous noise to the true function value
noise.sd=rep(n.level*(max(y_all)-min(y_all)),nrow(x_all))
# Making noisy observations from function 
set.seed(4)
mea <- y_all + noise.sd*rnorm(nrow(x_all))
data_all=cbind(mea,x_all,noise.sd)
#plot(unlist(x_all),unlist(mea))
#par(new=T)
#plot(unlist(x_all),unlist(mea),col="red")
...
write.csv(error,"error_true noise0.1.csv")
