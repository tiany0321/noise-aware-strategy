setwd("D:\\as-code\\latent heat\\strategy II")
#####library
library(DiceKriging)

phi_opt=0.1#######the optimal noise level prefactor
...
EI= fn.utility("SKO",data.train,gp,err.data, gp.total,vir.x)
colnames(EI)="SKO"
##############data frame combining x, utility value, predicted mean and uncertainties in virtual space
data.SKO=cbind(vir.comp,gp,EI)
data.SKO=data.SKO[order(-data.SKO[,"SKO"]),]#######Sort the data from high to low according to the utility values
data.SKO=data.SKO[1:10000,]

###########the one ranked first is the recommended alloy
data.SKO[1,]

write.csv(data.SKO,"data.SKO.csv")
